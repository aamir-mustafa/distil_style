...
state_dict = {k:v for k,v in torch.load('checkpoints/ckpts_jungles_AIO/CINN_epoch_75.pt').items() if 'tmp_var' not in k}
...
s2= st.slider("Latent code 1", min_value= -1.0, max_value=1.0,  value= 0.03 , step=0.02 )
s3= st.slider("Latent code 2", min_value= -1.0, max_value=1.0,  value= 0.13 , step=0.02 )
...
#    
#    z, log_j = cinn(x, l)
    z_new= np.array([att_1, s2/-100, s3/10]).astype('float32')
